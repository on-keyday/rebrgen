...
def generate_web_glue_files(config_dir, output_dir):
    ...
    for config_file in config_files:
        ...
        WORKER_FACTORY += f"    \"{web_glue['lang_name']}\": () => new JobManager(new Worker(new URL('./{web_glue['worker_name']}_worker.js', import.meta.url))),\n".encode()
        ...
        COPY_WASM += f"copyWasm('bmgen/{web_glue['worker_name']}_worker.js');\n".encode() # also copy worker js
    UI_CANDIDATES += b"]);\n"
    WORKER_FACTORY += b"});\n"
    LSP_MAPPER += b"});\n"
    WORKER_FACTORY += b"""
