...
namespace ebmgen {
    ...
    */
    ...
    expected<void> derive_array_setter(TransformContext& tctx) {
        ...
        auto derive_setter = [&](const BranchInfo& field_info, const ArrayLengthInfo& array_info, ebm::ExpressionRef value) -> expected<ebm::StatementRef> {
            // copy of value
            // constructs:
            ...
            MAYBE(type, get_single_type(ebm::TypeKind::PROPERTY_SETTER_RETURN, ctx));
            EBM_SETTER_STATUS(status, type, ebm::SetterStatus::FAILED);
            EBM_RETURN(return_stmt, status, field_info.func_ref);
            ...
            // length_field = (cast)value.length;
            ebm::Block new_body;
            new_body.container.push_back(length_check_statement);
            new_body.container.push_back(field_info.assign_ref);
            new_body.container.push_back(length_assign);
        ...
        for (size_t i = 0; i < max_index; i++) {
            ...
            MAYBE(new_block_ref, derive_setter(field_info->second, array_info->second, value_ref));
