...
1.  **Run `python script/unictest.py` (or `python script/unictest.py --print-stdout` for debugging)**: This script automates the process of generating EBM, running the code generator, and testing it. It will also reveal unimplemented visitor hooks.
...
##### Visitor Hook Overview

- **What it is**: A visitor hook is a `.hpp` file containing a C++ code snippet. It is **not** a class definition. Instead, it's the literal body of a function that is dynamically generated by `ebmcodegen`.
- **Mechanism**: `ebmcodegen` generates functions like `visit_Statement_WRITE_DATA(...)`. The content of your `.hpp` file (e.g., `visitor/Statement_WRITE_DATA.hpp`) is `#include`d directly into the body of that function.
- **Return Value**: The function, and therefore your hook code, **must** return a value of type `expected<Result>`. The `Result` struct holds the generated code string in its `value` member.

##### Managing Visitor Hooks with `ebmtemplate.py`

The `script/ebmtemplate.py` script is a wrapper around `ebmcodegen` that simplifies the lifecycle of visitor hooks.

**Usage:**
`python script/ebmtemplate.py <command> [args]`

**Commands:**

1.  **`python script/ebmtemplate.py <template_target> <lang>`**

    - **Purpose**: Creates a new visitor hook file.
    - **Action**: Generates a file at `src/ebmcg/ebm2<lang>/visitor/<template_target>.hpp`.
    - **Content**: The file contains a comment block listing all variables available within the hook's context, followed by `/*here to write the hook*/`.
    - **Example**:
      ```bash
      # Creates src/ebmcg/ebm2python/visitor/Statement_WRITE_DATA.hpp
      python script/ebmtemplate.py Statement_WRITE_DATA python
      ```

2.  **`python script/ebmtemplate.py update <lang>`**
    - **Purpose**: Updates the auto-generated comment blocks in all existing hooks for a given language.
    - **Action**: It re-generates the variable information for each hook in `src/ebmcg/ebm2<lang>/visitor/` and replaces the old comment block. Your implementation code below the comments is not affected.
    - **When to use**: Run this after the EBM structure (`extended_binary_module.bgn`) has been modified to ensure your hooks have access to the latest variable information.

##### Workflow for Implementing a Visitor Hook

1.  **Find the Target**: Decide which EBM node you want to handle.

    ```bash
    tool/ebmcodegen --mode hooklist
    ```

2.  **Create the File**: Generate the hook file using the target name and your language.

    ```bash
    python script/ebmtemplate.py Expression_CONDITIONAL python
    ```

3.  **Implement the Logic**: Open the newly created file (`src/ebmcg/ebm2python/visitor/Expression_CONDITIONAL.hpp`). Replace the `/*here to write the hook*/` comment with your C++ implementation.

    **Example: `Expression_CONDITIONAL.hpp`**

    ```cpp
    /* (Auto-generated comment block with available variables is here) */

    // This code is the body of a function that must return expected<Result>.
    // The variables 'condition', 'then', and 'else_' are available here.
    ...
    // The .value member of the Result object holds the generated string.
    auto generated_code = std::format("({} if {} else {})",
                                      then_str.value,
                                      cond_str.value,
                                      else_str.value);
