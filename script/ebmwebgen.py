...
import * as ebmgen  from "./{web_glue["wasm_glue_name"]}.js";
...
"""
            ...
            if "argdesc" in flag and flag["argdesc"]: # Check if it's a flag that takes an argument
...
const generate{upper_lang_name} = async (factory,traceID,opt,sourceCode) => {{
    ...
    const req = worker_mgr.getRequest(traceID,"{worker_name}",sourceCode);
...
"""
    for flag in flags:
        ...
        flag_name = flag["name"]
        flag_var_name = flag_name.replace("-", "_")
        code += f"""    opt.{flag_var_name} = ui.getLanguageConfig("{worker_name}","{flag_name}");
"""
    code += f"""    return opt;
}};
...
def generate_web_glue_files(tools :list[dict],output_dir):
    ...
    for tool in tools:
        ...
        UI_CALLS += f"    case \"{web_glue['worker_name']}\": return {web_glue['call_worker_func']}(factory,traceID,{web_glue["call_ui_to_opt_func"]}(ui),sourceCode);\n".encode()
        UI_CANDIDATES += f"\"{web_glue['worker_name']}\", ".encode()
        WORKER_FACTORY += f"    \"{web_glue['worker_name']}\": () => new JobManager(new Worker(new URL('./{web_glue['worker_name']}_worker.js', import.meta.url))),\n".encode()
        UI_SETS += f"  {web_glue['call_set_ui_func']}(ui);\n".encode()
        LSP_MAPPER += f"  \"{web_glue['worker_name']}\": \"{web_glue['lsp_lang']}\",\n".encode()
        COPY_WASM += f"copyWasm('bmgen/{web_glue['wasm_glue_name']}.js');\n".encode()
        COPY_WASM += f"copyWasm('bmgen/{web_glue['wasm_glue_name']}.wasm');\n".encode()
        COPY_WASM += f"copyWasm('bmgen/{web_glue['wasm_glue_name']}_worker.js');\n".encode() # also copy worker js
    UI_CANDIDATES += b"]);\n"
    WORKER_FACTORY += b"});\n"
    LSP_MAPPER += b"});\n"
    WORKER_FACTORY += b"""
...
""")
        ...
        f.write(
            b"export function generateEBMCode(ui,traceID,lang,sourceCode) {"
            + b" switch(lang) {\n"
            + UI_CALLS
        ...
        f.write(LSP_MAPPER)
        f.write(
            b"export function setEBMUIConfig(ui) {\n"
            + UI_SETS
            + b"}\n"
        )
